...
The `perform_linear`, `perform_logarithmic`, `perform_power` and `perform_exponential` matchers are useful in estimating the asymptotic behaviour of a given block of code. The most basic way to use the expectations to test how your code scales is to do following:
...
However, for the matchers to be of any use you will need to provide the range of inputs on which they will perform measurements. You can create a power range of inputs using `within` matcher:

```ruby
expect { ... }.to perform_linear.within(8, 100_000)
```

By default the range will be generated by powers of 8. You can change this using `:ratio` keyword:

```ruby
expect { ... }.to perform_linear.within(8, 100_000, ratio: 2)
```
