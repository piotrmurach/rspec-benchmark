...
The `perform_constant`, `perform_linear`, `perform_logarithmic`, `perform_power` and `perform_exponential` matchers are useful in estimating the asymptotic behaviour of a given block of code. The most basic way to use the expectations to test how your code scales is to do following:
...
However, for the matchers to be of any use you will need to provide the range of inputs on which they will perform measurements using `within` matcher. Each range input together with its corresponding iteration index will be yielded as arguments to the evaluted block. For example, to create a power range of inputs from `8` to `100_000` do:

```ruby
expect { |n, i|
  ...
}.to perform_linear.within(8, 100_000)
```

This will generate and yield input and index pairs [8, 0], [64, 1], [512, 2], [4K, 3], [32K, 4] and [100K, 5] respectively.

By default the range will be generated by powers of 8. You can change this using `:ratio` keyword:

```ruby
expect { |n, i|
  ...
}.to perform_linear.within(8, 100_000, ratio: 2)
```

Each range input is measured only once, you change this and thus increase tests stability using `sample` matcher. For example, to repeat measurements 100 times for each range data input do:

```ruby
expect { |n, i|
  ...
}.to perform_linear.within(8, 100_000, ratio: 2).sample(100).times
```
